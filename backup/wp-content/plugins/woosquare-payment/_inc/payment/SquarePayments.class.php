...
class WooSquare_Payments {
	...
	public function init() {
		...
		if(@$woocommerce_square_settings['enable_sandbox'] != 'yes'){
			if (!defined('SQUARE_APPLICATION_ID')) define('SQUARE_APPLICATION_ID',WOOSQU_PAY_APPID);
	...
	public function add_capture_charge_order_action( $actions ) {
		...
		$actions['square_capture_charge'] = esc_html__( 'Capture Charge', 'wpexpert-square' );
			...
			try {
				...
				if ( 'AUTHORIZED' === $transaction_status ) {
					...
					if ( is_wp_error( $result ) ) {
						$order->add_order_note( __( 'Unable to capture charge!', 'wpexpert-square' ) . ' ' . $result->get_error_message() );

						throw new Exception( $result->get_error_message() );
					} elseif ( ! empty( $result->errors ) ) {
						$order->add_order_note( __( 'Unable to capture charge!', 'wpexpert-square' ) . ' ' . print_r( $result->errors, true ) );
						...
						$order->add_order_note( sprintf( __( 'Square charge complete '.$msg.' (Charge ID: %s)', 'wpexpert-square' ), $trans_id ) );
						update_post_meta( $order->id, '_square_charge_captured', 'yes' );
						$this->log( "Info: Capture successful for {$order_id}" );
					}
				}
			} catch ( Exception $e ) {
				$this->log( sprintf( __( 'Error unable to capture charge: %s', 'wpexpert-square' ), $e->getMessage() ) );
			}
		}
	}
		...
		if ( 'square' === ( version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->payment_method : $order->get_payment_method() ) ) {
			try {
				...
				if ( 'AUTHORIZED' === $transaction_status ) {
					...
					if ( is_wp_error( $result ) ) {
						$order->add_order_note( __( 'Unable to void charge!', 'wpexpert-square' ) . ' ' . $result->get_error_message() );
						throw new Exception( $result->get_error_message() );
					} elseif ( ! empty( $result->errors ) ) {
						$order->add_order_note( __( 'Unable to void charge!', 'wpexpert-square' ) . ' ' . print_r( $result->errors, true ) );

						throw new Exception( print_r( $result->errors, true ) );
					} else {
						$order->add_order_note( sprintf( __( 'Square charge voided! (Charge ID: %s)', 'wpexpert-square' ), $trans_id ) );
						delete_post_meta( $order_id, '_square_charge_captured' );
						delete_post_meta( $order_id, 'woosquare_transaction_id' );
					}
				}
			} catch ( Exception $e ) {
				$this->log( sprintf( __( 'Unable to void charge!: %s', 'wpexpert-square' ), $e->getMessage() ) );
			}
		}
	}
