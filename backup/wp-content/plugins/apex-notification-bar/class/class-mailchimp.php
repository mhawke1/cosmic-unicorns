...
class APEXNB_MailChimp {
	/**
	 * @var string
	 */
	protected $transient_name = 'edn_mailchimp_lists';
	...
	public function get_subscriber_count( $list_ids ) {
		...
		$list_counts = get_transient( 'edn_list_counts' );
		...
		if( false === $list_counts ) {
			...
			if ( is_array( $lists ) ) {
				...
				$transient_lifetime = apply_filters( 'edn_lists_count_cache_time', 1200 ); // 20 mins by default

				set_transient( 'edn_list_counts', $list_counts, $transient_lifetime );
				set_transient( 'edn_list_counts_fallback', $list_counts, 86400 ); // 1 day
			} else {
				// use fallback transient
				$list_counts = get_transient( 'edn_list_counts_fallback' );
				if ( false === $list_counts ) {
					return 0;
				}
			}
		...
		return apply_filters( 'edn_subscriber_count', $count );
	}
