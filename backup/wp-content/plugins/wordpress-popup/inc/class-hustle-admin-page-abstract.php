...
if ( ! class_exists( 'Hustle_Admin_Page_Abstract' ) ) :
	...
	abstract class Hustle_Admin_Page_Abstract {
		...
		protected function render_modals() {
			// Force displaying the Black Friday sale modal.
			if ( ! filter_input( INPUT_GET, 'show-sale-modal', FILTER_VALIDATE_BOOLEAN ) ) {
				// Return, if not free, or not super admin, or don't have the admin privileges.
				if (
					! Opt_In_Utils::_is_free() ||
					! current_user_can( 'edit_others_posts' ) ||
					! is_super_admin() ||
					Hustle_Notifications::was_notification_dismissed( 'black-friday-sale-modal' )
				) {
					return;
				}
			}

			$this->get_renderer()->render( 'admin/dialogs/black-friday' );
		}
	}
