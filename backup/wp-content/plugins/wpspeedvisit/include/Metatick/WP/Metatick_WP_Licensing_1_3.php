...
if( ! class_exists('Metatick_WP_Licensing_1_3') ){
	class Metatick_WP_Licensing_1_3{
		...
		public function __construct($product,$secret,$name,$per_site_licensing = false){
			$this->product = $product;
			$this->secret = $secret;
			$this->name = $name;
			$this->per_site_licensing = $per_site_licensing;
			add_action( 'admin_notices', array($this,'admin_notices') );
			if ( ! wp_next_scheduled( 'Metatick_WP_Licensing_1_3_do_check' ) ) {
				wp_schedule_event( time(), 'daily', 'Metatick_WP_Licensing_1_3_do_check' );
			}
			add_action( 'Metatick_WP_Licensing_1_3_do_check', array($this,'update'));
		}
