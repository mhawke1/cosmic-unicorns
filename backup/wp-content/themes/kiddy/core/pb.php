...
function cws_mb_fillMbAttributes($meta, &$attr, $prefix = '') {
	$non_grouped = array('group', 'media', 'taxonomy');
...
function cws_mb_print_layout ($layout, $prefix) {
	$out = '';
	$isTabs = false;
	$tabs = array();
	...
	foreach ($layout as $key => $v) {
		...
		switch ($v['type']) {
			...
			case 'group':
				...
				$out .= cws_mb_print_layout( $v['layout'], $prefix . $name . '[%d][' ); // here would be a template stored
				$out .= '</textarea>';
			...
			case 'tab':
				...
				$out .= cws_mb_print_layout( $v['layout'], $prefix );
				$out .= '</div>';
				break;
			case 'textarea':
			...
			case 'button':
				...
				break;
			case 'datetime_add':
				$out .= '<ul class="recurring_events" data-pattern="'. $prefix . $name .'" data-lang="'. esc_html__('From', 'kiddy') . '|' . esc_html__('till', 'kiddy') .'">';
				if (!empty($v['source'])) {
					$i = 0;
					foreach ($v['source'] as $dstart => $dend) {
						$out .= '<li class="recdate">'. esc_html__('From', 'kiddy') .' <span>'.$dstart.'</span> '.esc_html__('till', 'kiddy').' <span>'. $dend .'</span><div class="close"></div>';
			...
			case 'taxonomy':
				...
				$out .= cws_mb_print_taxonomy($taxonomy, $v['source']);
				$out .= '</select>';
				break;
			case 'input_group':
			...
			case 'select':
				...
				if (!empty($v['source'])) {
					...
					if ( is_string($source) ) {
						...
						$out .= call_user_func_array('cws_mb_print_' . $func, array($arg0) );
					}
					else {
						foreach ($source as $key => $value) {
				...
				break;
			case 'media':
				//$out .= '<label for="cws-pb-row-img">' . esc_html__('Add background image', 'kiddy') . '</label>';
				//$media = isset($v['media']) ? ' data-media="' . $v['media'] . '"' : '';
				$isValueSet = !empty($v['value']['src']);
				$display_none = ' style="display:none"';
				$out .= '<div class="img-wrapper">';
				$out .= '<a class="pb-media-cws-pb"'. ($isValueSet ? $display_none : '') .'>'. esc_html__('Select', 'kiddy') . '</a>';
				$out .= '<a class="pb-remov-cws-pb"'. ($isValueSet ? '' : $display_none) .'>' . esc_html__('Remove', 'kiddy') . '</a>';
			...
			case 'gallery':
				...
				$out .= '<div class="img-wrapper">';
				$out .= '<a class="pb-gmedia-cws-pb">'. esc_html__('Select', 'kiddy') . '</a>';
...
function cws_mb_print_sidebars($sel) {
...
function cws_mb_print_taxonomy($name, $src) {
	$source = cws_mb_get_taxonomy_array($name);
...
function cws_mb_get_taxonomy_array($tax, $args = '') {
/*	if (!empty($args)) {
		$args .= '&';
	}
	$args .= 'hide_empty=0';*/
...
function cws_mb_print_fa ($sel) {
	$source = call_user_func(KIDDY_SLUG . '_get_all_fa_icons');
	$output = '<option value=""></option>';
	foreach($source as $k=>$v) {
		$selected = ($sel === $v) ? ' selected' : '';
		$output .= '<option value="' . $v . '" '.$selected.'>' . $v . '</option>';
	}
	return $output;
}

function cws_mb_print_titles ( $ptype ) {
	global $post;
	$output = '';
	$post_bc = $post;
