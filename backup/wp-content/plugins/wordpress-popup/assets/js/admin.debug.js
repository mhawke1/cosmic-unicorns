...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    } // Only display the Black Friday modal when other global modals aren't present.
    // TODO: refactor this.
    // Other modals are only shown on the dashboard page. This will change.


    if ($('#hustle-dialog--black-friday-sale').length && !$('#hustle-dialog--release-highlight').length && !$('#hustle-dialog--welcome').length && !$('#hustle-dialog--migrate').length) {
      SUI.openModal('hustle-dialog--black-friday-sale', $('.sui-header-title'));
      $('#hustle-dialog--black-friday-sale').on('close', function () {
        $.post(ajaxurl, {
          action: 'hustle_dismiss_notification',
          name: 'black-friday-sale-modal',
          _ajax_nonce: optinVars.dismiss_notice_nonce
        });
      });
    }
