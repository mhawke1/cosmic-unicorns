...
if ( !shortcode_exists( 'cws-widget' ) ) {
	...
	if (!$pbf_redeclares || !in_array( THEME_SLUG.'_cws_pricecol_renderer', $pbf_redeclares) ) {
...
if (!$pbf_redeclares || !in_array( THEME_SLUG.'_cws_text_renderer', $pbf_redeclares) ) {
...
if (!$pbf_redeclares || !in_array( THEME_SLUG.'_cws_tabs_renderer', $pbf_redeclares) ) {
...
if ( !shortcode_exists( 'item' ) ) {
	if (!$pbf_redeclares || !in_array(THEME_SLUG.'_cws_item_shortcode', $pbf_redeclares) ) {
		function cws_item_shortcode0 ($atts, $content) {
			extract( shortcode_atts(
				array(
						'type' => NULL,
						'open' => NULL,
	...
	add_shortcode( 'item', 'cws_item_shortcode0' );
}
...
function cws_pb_ourteam_shortcode($args) {
	...
	$pb_options = function_exists(THEME_SLUG.'_get_pb_options') ? call_user_func(THEME_SLUG.'_get_pb_options') : null;
	$staff_post_type = ($pb_options && isset($pb_options['ourteam']['options']['post_type'])) ? $pb_options['ourteam']['options']['post_type']  : 'staff';
	$staff_tax = ($pb_options && isset($pb_options['ourteam'])) ? $pb_options['ourteam']['options']['taxonomy']  : 'cws-staff-dept';
...
function cws_pb_tweet_shortcode($args) {
	...
	$twt_obj = call_user_func(THEME_SLUG . '_getTweets', '', $items, false );
...
if (!$pbf_redeclares || !in_array(THEME_SLUG.'_cws_tab_item_handler', $pbf_redeclares) ) {
...
if (!$pbf_redeclares || !in_array(THEME_SLUG.'_cws_accordion_renderer', $pbf_redeclares) ) {
...
if (!$pbf_redeclares || !in_array(THEME_SLUG.'_cws_callout_renderer', $pbf_redeclares) ) {
...
if (!$pbf_redeclares || !in_array(THEME_SLUG.'_cws_accordion_item_renderer', $pbf_redeclares) ) {
