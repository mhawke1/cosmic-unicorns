...
Plugin URI: http://pb.cwsthemes.com/
Description: internal use for CWSThemes only.
Text Domain: cws_pb
Version: 2.0.6
*/

define( 'CWS_PB_VERSION', '2.0.6' );
...
if (!defined('CWS_PB_HOST'))
	define('CWS_PB_HOST', 'http://up.cwsthemes.com/cwsbuilder/2');
...
add_filter( 'use_block_editor_for_post_type', 'cws_check_gut', 10, 2 );

function cws_check_gut($r, $post_type) {
	global $post;
	if(!empty($post->ID)){
		if ( isset( $_GET['classic-editor'] ) ) {
			update_post_meta( $post->ID, 'cwspb_isclassic', '1');
			return false;
		} else {
			$isclassic = get_post_meta( $post->ID, 'cwspb_isclassic', true );
			if ('1' === $isclassic) {
				return false;
			}
		}		
