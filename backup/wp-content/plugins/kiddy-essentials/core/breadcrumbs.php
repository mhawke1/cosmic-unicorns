<?php
function kiddy_breadcrumbs() {
	/* === OPTIONS === */
	$text['home']	 = esc_html__( 'Home', 'kiddy-essentials' ); // text for the 'Home' link
	$text['category'] = esc_html__( 'Archive by Category "%s"', 'kiddy-essentials' ); // text for a category page
	$text['search']   = esc_html__( 'Search for "%s"', 'kiddy-essentials' ); // text for a search results page
	$text['taxonomy'] = esc_html__( 'Archive by %s "%s"', 'kiddy-essentials' );
	$text['tag']	  = esc_html__( 'Posts Tagged "%s"', 'kiddy-essentials' ); // text for a tag page
	$text['author'] = esc_html__( 'Articles Posted by %s', 'kiddy-essentials' ); // text for an author page
	$text['404']	  = esc_html__( 'Error 404', 'kiddy-essentials' ); // text for the 404 page
	...
	$delimiter	  = '<i class="delimiter fa fa-chevron-' . ( is_rtl() ? 'left' : 'right' ) . '"></i>'; // delimiter between crumbs
	...
	if ( is_home() || is_front_page() || (empty( $post )) ) {
		...
		if ( is_category() ) {
			...
			if ( $show_current == 1 ) {
				echo $has_parent_cat ? $delimiter : '';
				echo $before . sprintf( $text['category'], $cat_name );
			}
		} elseif ( is_tag() ) {
			...
			echo $before . esc_html( sprintf( $text['author'], $userdata->display_name ) ) . $after;
			...
			if ( isset( $tax_obj->hierarchical ) && $tax_obj->hierarchical ) {
				...
				for ( $i = 0; $i < count( $parent_terms ); $i++ ) {
					...
					if ( ! empty( $cur_term_name ) && isset( $cur_term->term_id ) ) {
						...
						echo $has_parent_term ? $delimiter : '';
			...
			if ( $show_current == 1 ) {
				...
				echo $has_parent_term ? $delimiter : '';
				echo $before . esc_html( sprintf( $text['taxonomy'], $singular_tax_label, $term_name ) );
			}
		} elseif ( is_archive() ) {
			if ( $show_current ) {
				...
				echo $before . esc_html($post_type_name) . $after;
			}
		} elseif ( is_search() ) {
		...
		if ( get_query_var( 'paged' ) ) {
			...
			echo $delimiter . esc_html__( 'Page', 'kiddy-essentials' ) . ' ' . get_query_var( 'paged' );
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) { echo ')'; }
		}
