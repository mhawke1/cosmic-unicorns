...
class WooSquare_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		$this->id		            = 'square';
		$this->method_title 	    = __( 'Square', 'wpexpert-square' );
		$this->method_description   = __( 'Square works by adding payments fields in an iframe and then sending the details to Square for verification and processing.', 'wpexpert-square' );
	...
	public function payment_fields() { ?>
		<fieldset class="wooSquare-checkout">
			...
			<p class="form-row form-row-wide">
				<label for="sq-card-number"><?php esc_html_e( 'Card Number', 'wpexpert-square' ); ?> <span class="required">*</span></label>
			...
			<p class="form-row form-row-first">
				<label for="sq-expiration-date"><?php esc_html_e( 'Expiry (MM/YY)', 'wpexpert-square' ); ?> <span class="required">*</span></label>
				<input id="sq-expiration-date" type="text" autocomplete="off" placeholder="<?php esc_attr_e( 'MM / YY', 'wpexpert-square' ); ?>" name="<?php echo esc_attr( $this->id ); ?>-card-expiry" />
			</p>

			<p class="form-row form-row-last">
				<label for="sq-cvv"><?php esc_html_e( 'Card Code', 'wpexpert-square' ); ?> <span class="required">*</span></label>
				<input id="sq-cvv" type="text" autocomplete="off" placeholder="<?php esc_attr_e( 'CVV', 'wpexpert-square' ); ?>" name="<?php echo esc_attr( $this->id ); ?>-card-cvv" />
			</p>

			<p class="form-row form-row-wide">
				<label for="sq-postal-code"><?php esc_html_e( 'Card Postal Code', 'wpexpert-square' ); ?> <span class="required">*</span></label>
				<input id="sq-postal-code" type="text" autocomplete="off" placeholder="<?php esc_attr_e( 'Card Postal Code', 'wpexpert-square' ); ?>" name="<?php echo esc_attr( $this->id ); ?>-card-postal-code" />
			</p>
		</fieldset>
	...
	public function payment_scripts() {
		...
		wp_register_script( 'wpexpert-square', WooSquare_PLUGIN_URL . '/_inc/js/SquarePayments.js', array( 'jquery', 'square' ), WooSquare_VERSION, true );
		...
		wp_localize_script( 'wpexpert-square', 'square_params', array(
			'application_id'               => SQUARE_APPLICATION_ID,
			'environment'                  =>  'production' ,
			'locationId'                   =>  $location,
			'cart_total'                   =>  $cart_total['total'] ,
			'get_woocommerce_currency'	   =>  get_woocommerce_currency() ,
			'placeholder_card_number'      => __( '•••• •••• •••• ••••', 'wpexpert-square' ),
			'placeholder_card_expiration'  => __( 'MM / YY', 'wpexpert-square' ),
			'placeholder_card_cvv'         => __( 'CVV', 'wpexpert-square' ),
			'placeholder_card_postal_code' => __( 'Card Postal Code', 'wpexpert-square' ),
		...
		wp_enqueue_script( 'wpexpert-square' );
	...
	public function process_payment( $order_id, $retry = true ) {
		...
		try {
			$data = array(
				...
				'note' => apply_filters( 'woocommerce_square_payment_order_note', 'WooCommerce: Order #' . (string) $order->get_order_number().' '.$first_name.' '.$last_name, $order ),
			);
			...
			$fields = array(
				...
				"amount_money" => array(
					  "amount" => round( $this->format_amount( $order->get_total(), $currency ) ,1),
					  "currency" => $currency
					),
				"source_id" => $nonce,
			...
			if ( 'CAPTURED' === $result->payment->card_details->status ) {
				...
				// Mark as on-hold
				$authorized_message = sprintf( __( 'Square charge authorized (Authorized ID: %s). Process order to take payment, or cancel to remove the pre-authorization.', 'wpexpert-square' ), $result->payment->id );
	...
	public function maybe_create_customer( $order ) {
		...
		$customer = array(
			...
			'reference_id' => ! empty( $user ) ? (string) $user : __( 'Guest', 'wpexpert-square' ),
		);
		...
		if ( $create_customer ) {
			...
			if ( is_wp_error( $result ) ) {
				$this->log( sprintf( __( 'Error creating customer: %s', 'wpexpert-square' ), $result->get_error_message() ) );
				$order->add_order_note( sprintf( __( 'Error creating customer: %s', 'wpexpert-square' ), $result->get_error_message() ) );
			}

			// we don't want to halt any processes here just log it
			if ( ! empty( $result->errors ) ) {
				$this->log( sprintf( __( 'Error creating customer: %s', 'wpexpert-square' ), print_r( $result->errors, true ) ) );
				$order->add_order_note( sprintf( __( 'Error creating customer: %s', 'wpexpert-square' ), print_r( $result->errors, true ) ) );
			}
			...
			if ( ! is_wp_error( $result ) && empty( $result->errors ) && ! empty( $user ) ) {
				...
				$order->add_order_note( sprintf( __( 'Customer created on Square: %s', 'wpexpert-square' ), $result->customer->id ) );
			}
		}
	}
					...
					if ( is_wp_error( $result ) ) {
						...
						if ( 'APPROVED' === $result->refund->status || 'PENDING' === $result->refund->status ) {
							$refund_message = sprintf( __( 'Refunded %s - Refund ID: %s - Reason: %s', 'wpexpert-square' ), wc_price( $result->refund->amount_money->amount / 100 ), $result->refund->id, $reason );
								update_post_meta($order_id, "refund_created_at", $result->refund->created_at);
								update_post_meta($order_id, "refund_created_id", $result->refund->id);
