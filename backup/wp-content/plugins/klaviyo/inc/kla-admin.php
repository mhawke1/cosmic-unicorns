...
class WPKlaviyoAdmin {
    ...
    function process_settings() {
        ...
        if (!empty($_POST['klaviyo_option_submitted'])) {
            ...
            if ($_GET['page'] == 'klaviyo_settings' && check_admin_referer('klaviyo-update-settings')) {
                ...
                if ( !$klaviyo_settings['klaviyo_subscribe_checkbox'] && $klaviyo_settings['klaviyo_newsletter_list_id'] ) {
                    $klaviyo_notification->admin_message( 'use_checkbox', 10 );
                }
        ...
        //TODO: Remove this variable and delete this notification once LIST ID for checkout checkbox is sunset, in favor of subscribe_checkbox in settings form
        $checkbox_sunset_message = 'We are improving our settings form to be more intuitive, for a better user experience. Klaviyo will be sunsetting use of `Klaviyo List ID for Email` field to determine showing the Email Subscribe checkbox on checkout page in favor of `Subscribe contacts` checkbox setting. This feature will be sunset on 15th May, 2021';
        $klaviyo_notification->admin_message($checkbox_sunset_message, 20);
